...
bindings, protected protocol documents, detached-record evidence fields,
facilitator execution-log schema, undeclared-orchestration prohibition,
content/authority guards, the six-row results inventory, mutually exclusive
human/synthetic entry branches, full-route stage and debrief boundaries,
immutable results before log close, later external closeout binding, and the
proof-gated one-page US Letter handoff contract.
The mutation suite refreshes ordinary packet checksums and confirms that a
positive control passes while structural and semantic regressions are
rejected. A checksum pass shows that the prepared source
